...
use crate::world::{ChunkEntities, ChunkChannel, LastChunk, chunk_view_generator, generate_mesh, generate_no_padding_dumby_chunk}; 
use std::sync::mpsc::{channel, Sender};
use std::sync::Mutex;


const CHUNK_LOAD_DISTANCE: i32 = 32;
...
fn prune_chunks(single: Single<Movement, With<Player>>, mut chunk_entities: ResMut<ChunkEntities>, mut commands: Commands){ 
...
fn load_chunks(single: Single<Movement, With<Player>>, mut materials: ResMut<Assets<StandardMaterial>>,
    mut meshes: ResMut<Assets<Mesh>>, chunk_channel: Res<ChunkChannel>, mut chunk_entities: Res<ChunkEntities>, mut commands: Commands){ 
    ...
    for x in -CHUNK_LOAD_DISTANCE..=CHUNK_LOAD_DISTANCE {for z in -CHUNK_LOAD_DISTANCE..=CHUNK_LOAD_DISTANCE {
...
fn process_chunk_meshes(
    ...
    while let Ok((chunk_pos, mesh)) = rx.try_recv() {
        let entity = commands.spawn((
            ...
            Transform::from_xyz(
                16.0 * chunk_pos.x as f32,
                0.0,
                16.0 * chunk_pos.z as f32
